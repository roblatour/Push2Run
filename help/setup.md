...
**Use with IFTTT and Push2Run** | each IFTTT applet allows you to identify on exactly which computer(s) you want a particular IFTTT triggered command to run<br>Note: an IFTTT Pro / Pro+ account is needed for this option| each IFTTT applet allows you to identify on exactly which computer(s) you want a particular IFTTT triggered command to run|each IFTTT applet triggers one or all Pushover devices, so if you are running Push2Run on three or more computers you may not be able to tailor exactly which computers you want a particular IFTTT triggered command to run
